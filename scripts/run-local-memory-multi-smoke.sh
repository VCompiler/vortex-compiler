...
usage() {
  cat <<'EOF'
Usage:
  run-local-memory-multi-smoke.sh \
    [--platform-root /path/to/vortex-platform] \
    [--output-dir build/smoke/local_memory_multi_i32] \
    [--driver simx|rtlsim] \
    [--sim-build-root build/vortex-sim-local-memory-multi] \
    [--build-sim] \
    [--build-third-party] \
    [--no-proxy] \
    [--make-var 'NAME=VALUE'] ... \
...
Builds and runs a multi-core/multi-warp/multi-lane Vortex local-memory smoke.
The default simulator config is 2 cores, 2 warps per core, and 4 threads per
warp. The generated assembly is checked to contain a CSR read of lmem_base and
no call to vx_local_mem_base.
EOF
}

log() {
  echo "[local-memory-multi-smoke] $*"
}

die() {
  echo "[local-memory-multi-smoke] error: $*" >&2
  exit 1
}
...
PLATFORM_ROOT=""
OUTPUT_DIR="${REPO_ROOT}/build/smoke/local_memory_multi_i32"
DRIVER="simx"
SIM_BUILD_ROOT="${REPO_ROOT}/build/vortex-sim-local-memory-multi"
BUILD_SIM=0
BUILD_THIRD_PARTY=0
NO_PROXY=0
VERBOSE=0
...
declare -a MAKE_VARS=(
  "CONFIGS=-DNUM_CORES=2 -DNUM_WARPS=2 -DNUM_THREADS=4"
)
declare -a SIM_ARGS=(
  -c 2
  -w 2
...
INPUT_MLIR="${REPO_ROOT}/examples/smoke/local_memory_multi_i32.mlir"
WRAPPER_C="${REPO_ROOT}/examples/smoke/local_memory_multi_i32_wrapper.c"
ELF_PATH="${OUTPUT_DIR}/local_memory_multi_i32.elf"
ASM_PATH="${OUTPUT_DIR}/local_memory_multi_i32.s"
...
log "building multi-lane local-memory smoke kernel"
run_cmd "${build_cmd[@]}"
...
log "running ${DRIVER} multi-lane smoke"
run_cmd "${sim_cmd[@]}"
