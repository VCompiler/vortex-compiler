// RUN: not vx-opt %s --vortex-materialize-simt-control-flow 2>&1 | FileCheck %s

func.func @bad_results() attributes {vortex.kernel} {
  %c1 = arith.constant 1 : index
  %c0 = arith.constant 0 : index
  vortex.launch %c1, %c1, %c1 {
    %tid = vortex.thread_id : index
    %pred = arith.cmpi eq, %tid, %c0 : index
    %value = scf.if %pred -> (i32) {
      %v0 = arith.constant 0 : i32
      scf.yield %v0 : i32
    } else {
      %v1 = arith.constant 1 : i32
      scf.yield %v1 : i32
    }
    %idx = arith.index_cast %value : i32 to index
    vortex.tmc %idx
    vortex.yield
  }
  return
}

// CHECK: error:
// CHECK-SAME: cannot materialize may-varying scf.if with results
