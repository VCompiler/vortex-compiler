...
# Usage: ./build_and_run.sh <kernel_name>
# Example: ./build_and_run.sh reduce_sum
#
# Expects:
#   examples/gpt2/<name>.mlir
#   examples/gpt2/<name>_wrapper.c

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd -P)"

NAME="${1:?Usage: $0 <kernel_name>}"
MLIR="${SCRIPT_DIR}/${NAME}.mlir"
WRAPPER="${SCRIPT_DIR}/${NAME}_wrapper.c"
OUTDIR="${REPO_ROOT}/build/gpt2/${NAME}"
...
clang-18 --target=riscv32 -march=rv32imaf -mabi=ilp32f \
  ...
  -c "$WRAPPER" -o "$OUTDIR/${NAME}_wrapper.o"
