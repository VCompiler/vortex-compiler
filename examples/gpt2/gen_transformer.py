...
def gen_wrapper_with_weights(S: int, D: int, FF: int, func_name: str,
                              weights_header: str, layers: int = 1) -> str:
    """Generate a wrapper that loads weights from a static const header,
    calls the kernel for each layer, and compares against golden output."""

    # Build the per-layer weight unpacking and kernel call chain
    layer_code_parts = []
  ...
  {{
    ...
    for (int j = 0; j < D; j++) ln1_gamma[j] = wp[off++];
    for (int j = 0; j < D; j++) ln1_beta[j]  = wp[off++];
    for (int j = 0; j < D * D; j++) wq[j] = wp[off++];
    for (int j = 0; j < D * D; j++) wk[j] = wp[off++];
    for (int j = 0; j < D * D; j++) wv[j] = wp[off++];
    for (int j = 0; j < D * D; j++) wo[j] = wp[off++];
    for (int j = 0; j < D; j++) ln2_gamma[j] = wp[off++];
    for (int j = 0; j < D; j++) ln2_beta[j]  = wp[off++];
    for (int j = 0; j < D * D_FF; j++) w1[j] = wp[off++];
    for (int j = 0; j < D_FF * D; j++) w2[j] = wp[off++];

    /* zero scratch buffers */
    for (int j = 0; j < S * D; j++) {{
      x_ln[j] = 0.0f; q[j] = 0.0f; k[j] = 0.0f; v[j] = 0.0f;
      attn[j] = 0.0f; attn_out[j] = 0.0f; x_ln2[j] = 0.0f;
      x_out[j] = 0.0f;
    }}
    for (int j = 0; j < S * S; j++) {{ score[j] = 0.0f; prob[j] = 0.0f; }}
    for (int j = 0; j < S * D_FF; j++) hidden[j] = 0.0f;
    for (int j = 0; j < S; j++) {{
      ln_mean[j] = 0.0f; ln_var[j] = 0.0f;
      sm_max[j]  = 0.0f; sm_sum[j] = 0.0f;
    }}

    /* call kernel: x_in -> x_out */
    {func_name}(
        x_in, x_out,
        ln1_gamma, ln1_beta,
        wq, wk, wv, wo,
    ...
    /* copy x_out -> x_in for next layer (if not last) */
    for (int j = 0; j < S * D; j++)
      x_in[j] = x_out[j];
  }}""")
...
int main() {{
  ...
  /* ---- load input from static const array ---- */
  for (int i = 0; i < S * D; i++)
    x_in[i] = input_data[i];
...
{layer_code}
  ...
  float max_diff = 0.0f;
  for (int i = 0; i < S * D; i++) {{
    float diff = x_out[i] - golden_output[i];
    if (diff < 0) diff = -diff;
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to weights.h header (generated by export_weights.py). "
             "When provided, the wrapper loads weights from static const arrays "
             "and compares against golden output instead of inline random init.",
    )
    parser.add_argument(
        "--layers",
        type=int,
        default=1,
        help="Number of transformer layers (used with --weights-header, default: 1)",
    )
    ...
    if args.weights_header:
        wrapper_content = gen_wrapper_with_weights(
            S, D, FF, func_name, args.weights_header, args.layers
        )
    else:
