...
# ---------------------------------------------------------------------------
# External weight loading: binary files + host driver
# ---------------------------------------------------------------------------

def write_weights_bin(path: str, token_ids: np.ndarray, tok_table: np.ndarray,
                      pos_table: np.ndarray, layers_weights: list,
                      final_ln_gamma: np.ndarray, final_ln_beta: np.ndarray,
                      lm_head_w: np.ndarray):
    """Write all weights as a single contiguous binary file (little-endian float32/int32)."""
    with open(path, "wb") as f:
        f.write(token_ids.astype(np.int32).tobytes())
        f.write(tok_table.astype(np.float32).tobytes())
        f.write(pos_table.astype(np.float32).tobytes())
        for weights in layers_weights:
            for key in WEIGHT_ORDER:
                f.write(weights[key].ravel().astype(np.float32).tobytes())
        f.write(final_ln_gamma.astype(np.float32).tobytes())
        f.write(final_ln_beta.astype(np.float32).tobytes())
        f.write(lm_head_w.astype(np.float32).tobytes())


def gen_common_header(S: int, D: int, FF: int, V: int, N: int) -> str:
    """Generate gpt2_common.h -- kernel_arg_t shared between host and kernel."""
    per_layer = 2 * D + 4 * D * D + 2 * D + D * FF + FF * D
    return f"""\
/* Auto-generated by gen_full_inference.py -- do not edit */
#ifndef GPT2_COMMON_H
#define GPT2_COMMON_H
...
#define GPT2_FF      {FF}
#define GPT2_VOCAB   {V}
#define GPT2_LAYERS  {N}
#define GPT2_LAYER_WEIGHT_FLOATS  {per_layer}
...
typedef struct {{
  ...
  uint32_t num_layers;
  uint32_t _pad;
  uint64_t weights_addr;  /* base address of weights blob in device DDR */
  uint64_t output_addr;   /* address for output logits [S*V] floats */
}} kernel_arg_t;
...
def gen_external_kernel(S: int, D: int, FF: int, V: int, N: int) -> str:
    """Generate gpt2_kernel.c -- RISC-V kernel that reads weights from DDR."""
    per_layer = 2 * D + 4 * D * D + 2 * D + D * FF + FF * D
    return f"""\
/* Auto-generated by gen_full_inference.py -- do not edit */
#include <vx_intrinsics.h>
#include <vx_print.h>
#include <string.h>
#include "gpt2_common.h"

/* ---- MLIR kernel declarations (bare-ptr calling convention) ---- */
...
int main() {{
  ...
  /* Read kernel arguments from CSR_MSCRATCH */
  kernel_arg_t* arg = (kernel_arg_t*)csr_read(0x340);

  /* Compute pointers into the DDR weight blob */
  char* base = (char*)(uint32_t)arg->weights_addr;
  uint32_t off = 0;

  int*   input_ids    = (int*)(base + off);   off += S * sizeof(int);
  float* tok_embed    = (float*)(base + off); off += V * D * sizeof(float);
  float* pos_embed    = (float*)(base + off); off += S * D * sizeof(float);

  uint32_t layers_base = off;
  uint32_t per_layer_bytes = GPT2_LAYER_WEIGHT_FLOATS * sizeof(float);
  off += {N} * per_layer_bytes;

  float* final_gamma  = (float*)(base + off); off += D * sizeof(float);
  float* final_beta   = (float*)(base + off); off += D * sizeof(float);
  float* lm_proj      = (float*)(base + off);

  float* output       = (float*)(uint32_t)arg->output_addr;
  ...
  for (int layer = 0; layer < {N}; layer++) {{
    ...
    float* lw = (float*)(base + layers_base + layer * per_layer_bytes);
    uint32_t wp = 0;
    float* ln1_gamma  = lw + wp; wp += D;
    float* ln1_beta   = lw + wp; wp += D;
    float* wq         = lw + wp; wp += D * D;
    float* wk         = lw + wp; wp += D * D;
    float* wv         = lw + wp; wp += D * D;
    float* wo         = lw + wp; wp += D * D;
    float* ln2_gamma  = lw + wp; wp += D;
    float* ln2_beta   = lw + wp; wp += D;
    float* w1         = lw + wp; wp += D * D_FF;
    float* w2         = lw + wp;

    /* Zero scratch buffers */
    memset(tb_x_out, 0, sizeof(tb_x_out));
    memset(tb_x_ln, 0, sizeof(tb_x_ln));
    memset(tb_q, 0, sizeof(tb_q));
    memset(tb_k, 0, sizeof(tb_k));
    memset(tb_v, 0, sizeof(tb_v));
    memset(tb_score, 0, sizeof(tb_score));
    memset(tb_prob, 0, sizeof(tb_prob));
    memset(tb_attn, 0, sizeof(tb_attn));
    memset(tb_attn_out, 0, sizeof(tb_attn_out));
    memset(tb_x_ln2, 0, sizeof(tb_x_ln2));
    memset(tb_hidden, 0, sizeof(tb_hidden));
    memset(tb_ln_mean, 0, sizeof(tb_ln_mean));
    memset(tb_ln_var, 0, sizeof(tb_ln_var));
    memset(tb_sm_max, 0, sizeof(tb_sm_max));
    memset(tb_sm_sum, 0, sizeof(tb_sm_sum));
    ...
    memcpy(x_cur, tb_x_out, sizeof(x_cur));
    vx_printf("layer %d done\\n", layer);
  }}
  ...
  memset(lm_ln_mean, 0, sizeof(lm_ln_mean));
  memset(lm_ln_var, 0, sizeof(lm_ln_var));

  lm_head(x_cur, final_gamma, final_beta, lm_proj,
          logits, lm_ln_out, lm_ln_mean, lm_ln_var);

  vx_printf("lm_head done\\n");
...
def gen_host_driver(S: int, D: int, FF: int, V: int, N: int) -> str:
    """Generate gpt2_host.cpp -- x86 host driver using vortex runtime API."""
    return f"""\
/* Auto-generated by gen_full_inference.py -- do not edit
 *
 * GPT-2 host driver: loads kernel + weights, runs on vortex, verifies output.
 *
 * Build:
 *   g++ -std=c++17 -O2 -I$PLATFORM/runtime/include -I$PLATFORM/hw \\
 *       gpt2_host.cpp -L$PLATFORM/runtime/simx/.. -lvortex-simx -lsimx -o gpt2_host
 *
 * Run:
 *   LD_LIBRARY_PATH=$PLATFORM/runtime/simx/.. ./gpt2_host \\
 *       -k kernel.bin -w weights.bin -g golden.bin
 */
#include <iostream>
#include <fstream>
#include <vector>
...
#define RT_CHECK(expr) do {{ \\
    int _r = (expr); \\
    if (_r != 0) {{ \\
      std::cerr << "ERROR: " << #expr << " returned " << _r << std::endl; \\
      cleanup(); \\
      return 1; \\
    }} \\
  }} while (0)
...
static vx_buffer_h weights_buffer = nullptr;
...
int main(int argc, char** argv) {{
  ...
  const char* golden_file = "golden.bin";

  int c;
  while ((c = getopt(argc, argv, "k:w:g:h")) != -1) {{
    switch (c) {{
    case 'k': kernel_file = optarg; break;
    case 'w': weights_file = optarg; break;
    case 'g': golden_file = optarg; break;
    case 'h':
      std::cout << "Usage: [-k kernel.bin] [-w weights.bin] [-g golden.bin]" << std::endl;
      return 0;
    default:
      return 1;
    }}
  ...
  auto weights_data = read_file(weights_file);
  auto golden_data  = read_file(golden_file);

  uint32_t output_size = {S} * {V} * sizeof(float);

  std::cout << "Weights: " << weights_data.size() << " bytes" << std::endl;
  std::cout << "Golden:  " << golden_data.size() << " bytes" << std::endl;
  std::cout << "Model:   seq={S}, dim={D}, ff={FF}, vocab={V}, layers={N}" << std::endl;
  ...
  std::cout << "Device cores: " << num_cores << std::endl;

  /* Upload kernel binary */
  std::cout << "Uploading kernel..." << std::endl;
  RT_CHECK(vx_upload_kernel_file(device, kernel_file, &krnl_buffer));

  /* Allocate device DDR for weights */
  std::cout << "Allocating device memory..." << std::endl;
  RT_CHECK(vx_mem_alloc(device, weights_data.size(), VX_MEM_READ, &weights_buffer));
  RT_CHECK(vx_mem_alloc(device, output_size, VX_MEM_WRITE, &output_buffer));

  uint64_t weights_addr, output_addr;
  RT_CHECK(vx_mem_address(weights_buffer, &weights_addr));
  RT_CHECK(vx_mem_address(output_buffer, &output_addr));

  std::cout << "  weights @ 0x" << std::hex << weights_addr
            << " (" << std::dec << weights_data.size() << " bytes)" << std::endl;
  std::cout << "  output  @ 0x" << std::hex << output_addr
            << " (" << std::dec << output_size << " bytes)" << std::endl;

  /* Copy weights to device DDR */
  auto t0 = std::chrono::high_resolution_clock::now();
  std::cout << "Uploading weights to device DDR..." << std::endl;
  RT_CHECK(vx_copy_to_dev(weights_buffer, weights_data.data(), 0, weights_data.size()));
  auto t1 = std::chrono::high_resolution_clock::now();
  double upload_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
  std::cout << "  Upload time: " << upload_ms << " ms" << std::endl;
  ...
  karg.vocab      = {V};
  karg.num_layers = {N};
  karg.weights_addr = weights_addr;
  karg.output_addr  = output_addr;

  RT_CHECK(vx_upload_bytes(device, &karg, sizeof(karg), &args_buffer));

  /* Execute kernel */
  std::cout << "Starting kernel execution..." << std::endl;
  auto t2 = std::chrono::high_resolution_clock::now();
  RT_CHECK(vx_start(device, krnl_buffer, args_buffer));
  RT_CHECK(vx_ready_wait(device, 24 * 60 * 60 * 1000));  /* 24h timeout */
  auto t3 = std::chrono::high_resolution_clock::now();
  double exec_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t3 - t2).count();
  std::cout << "  Execution time: " << exec_ms << " ms" << std::endl;

  /* Read back output logits */
  std::vector<float> output_logits(output_size / sizeof(float));
  RT_CHECK(vx_copy_from_dev(output_logits.data(), output_buffer, 0, output_size));

  /* Verify against golden */
  const float* golden = reinterpret_cast<const float*>(golden_data.data());
  uint32_t total = {S} * {V};
  int pass = 1;
  float max_diff = 0.0f;
  for (uint32_t i = 0; i < total; i++) {{
    float diff = std::fabs(output_logits[i] - golden[i]);
    if (diff > max_diff) max_diff = diff;
  ...
  if (pass) {{
    ...
    std::cout << "FAILED (max_diff=" << max_diff << ")" << std::endl;
    /* Print first few mismatches */
    int shown = 0;
    for (uint32_t i = 0; i < total && shown < 10; i++) {{
      float diff = std::fabs(output_logits[i] - golden[i]);
      if (diff > 5e-2f) {{
        std::cout << "  [" << i << "] got=" << output_logits[i]
                  << " expected=" << golden[i] << " diff=" << diff << std::endl;
        shown++;
      }}
    }}
  }}
...
def main():
    ...
    external = args.external_weights

    # 1. MLIR (always generated)
    ...
    if external:
        # --- External weights mode ---

        # 2a. Binary weights blob
        weights_bin_path = os.path.join(out_dir, "weights.bin")
        write_weights_bin(weights_bin_path, token_ids, tok_table, pos_table,
                          all_layers_weights,
                          final_ln_gamma, final_ln_beta, lm_head_w)
        print(f"Wrote {weights_bin_path} ({os.path.getsize(weights_bin_path)} bytes)")

        # 2b. Golden logits binary
        golden_bin_path = os.path.join(out_dir, "golden.bin")
        with open(golden_bin_path, "wb") as f:
            f.write(golden_logits_arr.ravel().astype(np.float32).tobytes())
        print(f"Wrote {golden_bin_path} ({os.path.getsize(golden_bin_path)} bytes)")

        # 3. Common header (kernel_arg_t)
        common_h = gen_common_header(S, D, FF, V, N)
        common_h_path = os.path.join(out_dir, "gpt2_common.h")
        with open(common_h_path, "w") as f:
            f.write(common_h)
        print(f"Wrote {common_h_path}")

        # 4. Kernel code (RISC-V)
        kernel_c = gen_external_kernel(S, D, FF, V, N)
        kernel_c_path = os.path.join(out_dir, "gpt2_kernel.c")
        with open(kernel_c_path, "w") as f:
            f.write(kernel_c)
        print(f"Wrote {kernel_c_path}")

        # 5. Host driver (x86)
        host_cpp = gen_host_driver(S, D, FF, V, N)
        host_cpp_path = os.path.join(out_dir, "gpt2_host.cpp")
        with open(host_cpp_path, "w") as f:
            f.write(host_cpp)
        print(f"Wrote {host_cpp_path}")

    else:
        # --- Embedded weights mode (original behavior) ---
    ...
    total_bytes = (S * 4 + total_weights * 4)  # int32 ids + float32 weights
    print(f"\nSummary: seq={S}, dim={D}, ff={FF}, vocab={V}, layers={N}, seed={seed}")
    print(f"  Mode: {'external weights' if external else 'embedded weights'}")
